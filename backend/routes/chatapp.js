...
module.exports.search = async function(req,res){
  if(req.body.email=='' || req.body.email==null)
   ...
   {
    var sql="SELECT * FROM chat WHERE reciever LIKE '"+req.body.email+"%' AND reciever !='"+req.body.e1+"' GROUP BY reciever";
    con.query(sql,function(err , data){
        if (err) {
         console.log(err);
          return res.status(400).json({code:0});
    ...
    module.exports.addmsg = async function(req,res){
      ...
      module.exports.sendermsg = async function(req,res){
        var users={
           ...
           var sql = "SELECT * FROM  chat WHERE sender ='"+users.sender+"' AND reciever ='"+users.reciever+"' OR sender ='"+users.reciever+"' AND reciever ='"+users.sender+"'ORDER BY date desc";
           var query = con.query(sql, function(err, result) {  
            if (err) {
              console.log(err);
              return res.json({code:0});
